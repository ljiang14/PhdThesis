...
\end{enumerate}
\pause
The \href{https://en.wikipedia.org/wiki/Law_of_large_numbers}{Law of Large Numbers} states that
\begin{equation*}
\hmu_n \to \mu \text{  as } \alert{n \to \infty}
\end{equation*}
}
...
\alert{n_{\Cheb}(\varepsilon_a/\sigma,\alpha):= \left \lceil \frac{1}{\alpha (\varepsilon_a/\sigma)^2} \right \rceil}.
 \end{align*}
 }
...
\end{theorem}
The Berry-Esseen Inequality leads to a confidence interval when
\begin{align*}
 \alert{n_{\BE}(\varepsilon_a/\sigma,\alpha,M_3) := \min  \left \{ n \in \naturals : \Phi\left(-\sqrt{n}\varepsilon_a/\sigma  \right)+\delta_n(\sqrt{n}\varepsilon_a/\sigma,M_3)
...
\frame{\frametitle{The algorithm {\tt meanMCabs\_g}}
User input a random number generator \alert{$Y$} and the absolute error tolerance \alert{$\varepsilon_a$}. With the parameters $\alpha$, $\alpha_\sigma$, $n_\sigma$ and $\fudge$, we first
\begin{enumerate}
...
\begin{align*}
n_\mu &=n_{\BE} \left(\varepsilon_a/\hsigma,\alpha_\mu, \kmax^{3/4}\right)\\&:=\min \left\{n_{\Cheb}\left(\varepsilon_a/\hsigma,\alpha_\mu \right),n_{\BE}\left(\varepsilon_a/\hsigma,\alpha_\mu, \kmax^{3/4}\right)\right\}
...
\caption{dimension $d$ is random chosen from $1, 2, \ldots, 8$ and absolute error tolerance is $\varepsilon_a=10^{-2}$. The solid line shows that cumulative distribution of actual errors, and the dot-dashed line shows the cumulative distribution of time. }
\end{figure}
...
\begin{figure}
\centering
\begin{minipage}{5.5cm} \centering \includegraphics[width=5.5cm]{KeistercubSobolErrTime-2015-09-13-00-24-12.eps} \\ {\tt cubSobol\_g} \end{minipage}
\begin{minipage}{5.5cm} \centering \includegraphics[width=5.5cm]{KeistercubLatticeErrTime-2015-09-13-00-24-12.eps} \\ {\tt cubLattice\_g} \end{minipage}
\end{figure}
...
\begin{figure}
\centering
\begin{minipage}{5.5cm} \centering \includegraphics[width=6cm]
...
\frame{\frametitle{Integrating a single hump}
\begin{figure}\label{fig:GaussianTestFun} 
\centering
\begin{minipage}{6cm} \centering \includegraphics[width=6cm]
{gaussiand=1iidErrTime-2015-08-12-16-25-16.eps} \\ {\tt cubMCabs\_g}  \end{minipage}
\caption{ Set $d=1$ and absolute error tolerance $\varepsilon_a=10^{-3}$. The solid line shows that cumulative distribution of actual errors, and the dot-dashed line shows the cumulative distribution of time.  The points labeled * are those with guarantees.}
\end{figure}
}
\frame{\frametitle{Integrating a single hump}
\begin{figure}
\centering
\begin{minipage}{5.5cm} \centering \includegraphics[width=6cm]{gaussiand=1cubSobolErrTime-2015-08-12-16-25-17.eps} \\  {\tt cubSobol\_g}\end{minipage}
\begin{minipage}{5.5cm} \centering \includegraphics[width=6cm]{gaussiand=1cubLatticeErrTime-2015-08-12-16-25-17.eps} \\ {\tt cubLattice\_g} \end{minipage}
\end{figure}
}
...
\item Else, define the next tolerance, $$\heps_{t+1} = \min\left(\heps_t/2, \max(\varepsilon_a,\theta\varepsilon_r\abs{\hmu_t})\right)$$ and compute the next sample size, $$n_{t+1} = n_{\CB}\left(\hvareps_{t+1}/\hsigma,\alpha_{t+1},\kappa_{\max}^{3/4}\right).$$ Increase $t$ by one and go back to step \ref{deltamu}. 
\end{enumerate}
}
...
where $\bar{\tau}(\beta):= \left \lceil 1+\log_2 \left(\frac{\left(\fudge^2+(\fudge^2-1)\sqrt{\frac{\alpha_{\sigma}(1-\beta)}{(1-\alpha_{\sigma})\beta}}\right) n_{\CB}^{-1}\left(n_1,\alpha_1,\kappa_{\max}^{3/4}\right)(1+\varepsilon_r)}{ \max(\varepsilon_a,\varepsilon_r \abs{\mu})}\right) \right \rceil.$
...
\frame{\frametitle{Proof of success of algorithm {\tt meanMC\_g}}
...
\frame{\frametitle{Proof of success of algorithm {\tt meanMC\_g}}
In the {\tt meanMC\_g}, several quantities are random, the random variable $Y$, the stopping time $\tau$, the sample size $n_i$,  the confidence interval width $\heps_i$ in each iteration, for $i = 1,2,\cdots, \tau$. 
...
\subsection{Numerical Examples for{\tt meanMC\_g}}
...
\begin{figure}
\centering
\begin{minipage}{5cm} \centering \includegraphics[width=5cm]{MVNPcubSobolErrTime-2015-09-17-23-49-44.eps} \\  {\tt cubSobol\_g}\end{minipage}
\begin{minipage}{5cm} \centering \includegraphics[width=5cm]{MVNPcubLatticeErrTime-2015-09-17-23-49-45.eps} \\ {\tt cubLattice\_g} \end{minipage}
\end{figure}
}
