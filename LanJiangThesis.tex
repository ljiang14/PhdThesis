...
If the random variable $Y$ is a Bernoulli one, which takes only two values, 0 and 1, with  $\Pr(Y=1) =p$. The case for computing the mean becomes estimating the probability $p = \e(Y)$. 
...
Unfortunately, this is an asymptotic result. The proposed procedure attains the desired coverage level in the limit as $\varepsilon\to 0$ but does not provide coverage guarantees for fixed $\varepsilon>0$.  We want such fixed $\varepsilon$ guarantees. 
...
\rho(y) = \frac{1}{2\sqrt{2\pi}}\left(\exp\left(-\frac{x^2}{2}\right)+\exp\left(-\frac{(x-200)^2}{2}\right)\right).
\end{align}
It is a mixture of two Gaussian distributions. Figure \ref{fig:gaussianmix} shows the probability density function. It is obvious that the data are clustered around two values, 0 and 200. This distribution is a heavy-tailed one. We are interested in calculating the random variable $Y$ with the distribution \eqref{phfy}, the problem can be written as:
$$\e(Y) = \int_{-\infty}^\infty y\rho(y)dy.$$
Using CLT and our guaranteed adaptive algorithm presented in this thesis,  
The absolute error tolerance and the uncertainty level are both $0.01$, 100 samples are used to estimate the variance. The CLT meets the error tolerance 70\% of the time, while the guaranteed adaptive algorithm meets the tolerance 98\% of the time. 

Why CLT fails?
The first reason is that the insufficient samples are used to estimate the variance, we do not have a conservative variance estimation to handle this heavy-tailed distribution. The other reason is, CLT uses an approximate error bound, not the guaranteed error bound.
...
\Chapter{Discussion and Future work}\label{chapter:comclusion}
