...
\Subsection{The sensitivity of parameters}\label{subsec:sensitivityofparameters}
...
\begin{figure}[htbp]
\centering
\begin{minipage}{7cm} \centering \includegraphics[width=7cm]{ParameterPlot/Ratio_BEn_sigtol_fixed_nsig1000-2016-02-06-00-28-38.eps}  \\ {$n_\sigma = 10^3$}  \end{minipage}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=8cm]{ParameterPlot/BE_CLT_n_ratio_diff_fudge-2016-02-06-00-27-09.eps} % requires the graphicx package
...
\end{align}
 ...
 \end{proposition}
 \begin{proof}
% Since the error bound \eqref{pointwiseerrorbound} hold,  adding $\Delta_-$ to $\hmu$ yields, 
% $$\mu - \heps+\Delta_-\leq \hmu+\Delta_- \leq \mu+ \heps+\Delta_-$$
% which is 
% \begin{multline}\label{relerrineq}
% \mu - \heps +\frac{1}{2} [\tol(\varepsilon_a, \varepsilon_r\abs{\hmu-\heps}) - \tol(\varepsilon_a, \varepsilon_r \abs{\hmu+\heps})] \leq \tmu  \\ \leq \mu+ \heps +\frac{1}{2} [\tol(\varepsilon_a, \varepsilon_r \abs{\hmu-\heps}) - \tol(\varepsilon_a, \varepsilon_r \abs{\hmu+\heps})].
% \end{multline}
%By the definition of $\tmu$,
% $$\tmu = \hmu+\Delta_-=\hmu+\frac{1}{2} [\tol(\varepsilon_a, \varepsilon_r \abs{\hmu-\heps}) - \tol(\varepsilon_a, \varepsilon_r \abs{\hmu+\heps})]$$
...
%
%In certain cases our procedure multiplies the computational cost by a large factor such as $2$ or $10$ shown in Figure \ref{fig:ratioBECLTsamplesize} compared to what one might spend based on the CLT with a known value of $\sigma$. While this seems inefficient, one should remember that the total elapsed time may still be well below several seconds.  Furthermore, one typically does not know $\sigma$ in advance, and our adaptive algorithm estimates $\sigma$ and then an appropriate sample size $n_\mu$ from the data.  Our algorithmic cost will be low when the unknown $\sigma$ is small and large when $\sigma$ is large.
