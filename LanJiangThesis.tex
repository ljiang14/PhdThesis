...
\Chapter{Guaranteed Monte Carlo method with an absolute error tolerance}
...
\end{align}
\end{proof}
\Chapter{Guaranteed Monte Carlo method with a generalized error tolerance}
...
\intertext{By the defination of stopping time $\tau$ and Lemma \ref{taufinite}, we would have:}
&\geq \Pr\left(\abs{\mu-\hmu_\tau}\leq \heps_\tau,  \Delta_{+,\tau} \geq \heps_{\tau}, \Delta_{+,k} < \heps_{k}, \text{for } k = 1,\cdots,\tau-1, \tau<\infty|\hsigma\geq \sigma\right)\Pr(\hsigma\geq \sigma)\\
...
&\geq\lim_{i\to \infty} \Pr\left(\abs{\mu-\hmu_j}\leq \heps_j, j<i|\hsigma \geq \sigma ,\tau < i\right)\Pr(\tau< i|\hsigma \geq \sigma)\Pr(\hsigma\geq \sigma)\\
&\geq \lim_{i\to \infty}\left (1-\sum_{j=1}^{i-1} \Pr\left(\abs{\mu-\hmu_j}> \heps_j|\hsigma \geq \sigma ,\tau < i\right)\right)\Pr(\tau< i|\hsigma \geq \sigma)\Pr(\hsigma\geq \sigma) \\
&\geq \lim_{i\to \infty}\left ( 1-\sum_{j=1}^{i-1}\alpha_i \right)\Pr(\tau< i|\hsigma \geq \sigma)(1-\alpha_\sigma) = \left(1-\sum_{j=1}^\infty\alpha_i\right)(1-\alpha_\sigma) = 1-\alpha
\end{align*}
...
%\end{align}
\end{proof}
\Chapter{Guranteed method for Bernoulli Random variables}
...
\Chapter{Guaranteed method for Numerical Integration via Monte Carlo sampling}
