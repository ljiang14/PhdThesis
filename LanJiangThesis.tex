...
Given the relative error tolerance, $\varepsilon_r$, we are looking into constructing the fixed width confidence interval for $p$ with the relative error criterion. By applying inequality \ref{ineq:Hoefftwoside} in Theorem \ref{hoeff}, we have
\begin{align*}
...
\end{align*}
Since $p$ is unknown, the lower bound is needed to ensure $n$ is large enough to bound the relative error. By applying one side Hoeffding's Inequality \ref{ineq:HoeffonesideA} in Theorem \ref{hoeff}, the following inequality holds:
...
n \geq -\frac{\log(\alpha)} {2\varepsilon^2 },
 \end{align*}
$p$ has a conservative lower bound $\hp_n-\varepsilon$.
