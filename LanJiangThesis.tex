...
Monte Carlo is versatile computational method that may be used to approximate the means, $\mu$, of random variables $Y$, whose distributions are not known explicitly. This thesis studies the ways to reliably construct a fixed width confidence interval for $\mu$ with some prescribed absolute error tolerance $\varepsilon_a$, relative error tolerance $\varepsilon_r$ and some generalized error criterion. The assumption is the kurtosis $\kappa$ of the random variable does not exceed a user specified bound $\kmax$. The key idea is to confidently estimate the variance of $Y$ in the first step by applying Cantelli's inequality. A Berry-Esseen inequality makes it possible to determine the sample size required to construct such a confidence interval. Several algorithms are developed and implemented, which included ones to calculate the mean of a random variable, perform numerical integration and estimate the probability. They have became part of Guaranteed Automatic Integration Library (GAIL).
...
\Section{Monte Carlo simulation} Monte Carlo method is a a scientific computing technique that relies on repeated re-sampling and statistical analysis to obtain the desired result, it uses some random numbers as a tool to calculate something that is not random. For example, we want to calculate the mean of a random variable $Y$, i.e. $\e(Y) = \mu$ where $\mu$ is a deterministic number.  One way to do it is sampling $n$ independent and identically distributed $X_i$ from the distribution of $X$ and take the sample average, 
...
\begin{align}\label{def:meanmcabsgtmu}
...
Let $Y$ be a random variable with mean $\mu$, variance $\sigma^2 >0$, and modified kurtosis $\kappa$. Let $\kmax=\kmax(n_\sigma,\alpha_\sigma,\fudge)$ be as defined in \eqref{def:kmax}. For any random variable $Y$ with a bounded kurtosis $\kappa\leq\kmax(\alpha_{\sigma},n_{\sigma},\fudge)$, Algorithm \ref{alg:meanMCabsg} above yields an estimate $\tmu$ given by \eqref{def:meanmcabsgtmu} that satisfies the fixed width confidence interval condition
\begin{align}
...
Since $\hsigma$ is random, $n_\mu$ as defined in \eqref{def:meanmcabsgnmu} is also random. Thus, the randomness of $\tmu$ comes both from the sample size $n_\mu$ and the random number generator $Y$. By adding the extra condition inside the probability, we have the inequality below:
\begin{align*}
&\Pr(\abs{\mu-\tmu} \leq \varepsilon_a) \\
&\geq  \Pr({\abs{\mu-\tmu} \leq \varepsilon_a}, {\hsigma >\sigma})\\
&= \Pr(\abs{\mu-\tmu} \leq \varepsilon_a |\hsigma>\sigma)\Pr(\hsigma >\sigma)\\
&\geq \Pr(\abs{\mu-\tmu} \leq \varepsilon_a |\hsigma > \sigma)(1-\alpha_\sigma).
\intertext{As the sample $n_\mu$ used to calculate $\tmu$ is defined by Berry Esseen inequality in \eqref{BEn}, we have:}
&\Pr(\abs{\mu-\tmu} \leq \varepsilon_a) \\
...
\intertext{Given $\hsigma >\sigma$, $\kappa \leq \kmax$, by applying Berry Esseen inequality result in \eqref{BEresult},  we have:}
&\Pr(\abs{\mu-\tmu} \leq \varepsilon_a) \\&\geq g\left(n_\mu,\hsigma, \kmax^{3/4}, \varepsilon_a\right)\left(1-\alpha_\sigma\right) \\&= (1-\alpha_\mu)(1-\alpha_\sigma) \\&= (1-\alpha).
\end{align*}
\end{proof}
...
In this chapter, we will illustrate another two algorithms, {\tt meanMC\_g} and {\tt cubMC\_g}, that are used to estimate the mean of a random variable and do numerical integration to some generalized error tolerance. The procedures were done in a loop until the stopping criteria are met. The detailed step of the algorithms were in Section \ref{sec:algmeanMCg} and \ref{sec:algcubmcg}, the computational complexity is in Section \ref{sec:meanmcgcost} and the numerical examples are in Section \ref{sec:cubmcgnumericalexample}.

\Section{A General Point-wise Error Criterion}\label{sec:generalerrorcriterion}
...
In this section, we introduce an algorithm that estimate the mean of a random variable to a specified hybrid error tolerance $\tol(\varepsilon_a, \varepsilon_r)$ defined in \ref{def:tolfun}. Before presenting our algorithm, we would introduce some notations. 
...
\end{gather}
where $\Phi$ is the standard Gaussian cumulative distribution function defined in \eqref{Def:PhiCDF} and $\delta$ is defined in \eqref{deltadef}. It then follows by Chebyshev's inequality and the Berry-Esseen Inequality that 
\begin{equation*}
\Prob[\abs{\hmu_n -\mu}<\hvareps] \geq 1-\alpha, \quad \text{provided } \kappa\leq\kmax, \text{ where }\hvareps=\sigma(Y) N_{CB}^{-1}(n,\alpha,\kappa_{\max}^{3/4}), 
\end{equation*} 
...
\item Else, define the next tolerance, $$\heps_{i+1} = \min\left(\heps_i/2, \max(\varepsilon_a,\alpha\varepsilon_r\abs{\hmu_i})\right)$$ where $\alpha = 95\%$ and the next sample size, $$n_{i+1} = N_{CB}\left(\hvareps_{i+1}/\hsigma,\alpha_{i+1},\kappa_{\max}^{3/4}\right).$$ Increase $i$ by one and go back to step \ref{deltamu}. 
\end{enumerate}
\end{enumerate}
...
\end{align}
 ...
 $$\heps_{i+1} = \min(\heps_i/2, \max(\varepsilon_a,\alpha\varepsilon_r\abs{\hmu_i})).$$ 
...
Let $Y$ be a random variable with mean $\mu$, and either zero variance or positive variance with modified kurtosis $\kappa \leq \kmax(\alpha_\sigma, n_\sigma, \fudge)$. Suppose $\varepsilon_a$ is random, it follows that Algorithm \ref{alg:meanMCabsg} above yields an estimate $\tmu$ given by \eqref{} that satisfies the fixed width confidence interval condition
\begin{align}
\Pr\left( \abs{\mu-\tmu} \leq \varepsilon_a \right) \geq 1-\alpha.
\end{align}
\end{lemma}
...
\begin{proof}
In the Algorithm \ref{alg:meanMCg}, several quantities are random, the stopping time $\tau$, the sample size $n_i$,  the confidence interval width $\heps_i$ in each iteration, for $i = 1,2,\cdots, \tau$. By Proposition \ref{meanMCgProp}, at step $\tau$, we know that
\begin{align}
...
\begin{align}
\heps_i= \min(\heps_{i-1}/2, \max(\varepsilon_a,\alpha\varepsilon_r\abs{\hmu_{i-1}})).
\end{align}
Define the lower bound of $\heps_i$ as
\begin{align}
  \underline{\varepsilon_i} = \min \left \{ \heps_12^{1-i},\varepsilon_a, \alpha\varepsilon_r\abs{\mu} \right \}.
\end{align}
It is obvious that
\begin{align}
&\heps_i \geq  \min(\heps_{i-1}/2, \varepsilon_a, \alpha\varepsilon_r\abs{\hmu_{i-1}})\\
& = \min(\heps_12^{1-i}, \varepsilon_a, \alpha\varepsilon_r\abs{\hmu_{i-1}})\\
\end{align}
...
\item Else, define the next tolerance, $$\heps_{i+1} = \min\left(\heps_i/2, \max\left(\varepsilon_a,\alpha\varepsilon_r\abs{\hat{I}_i}\right)\right)$$ where $\alpha=95\%$, and the next sample size, $$n_{i+1} = N_{CB}\left(\hvareps_{i+1}/\hsigma,\alpha_{i+1},\kappa_{\max}^{3/4}\right).$$ Increase $i$ by one and go back to step \ref{deltamu}. 
\end{enumerate}
\end{enumerate}
If this algorithm terminates, then the general error criterion, \eqref{cubMCgCI}, is satisfied.
\end{algorithm}

\Section{Numerical example of {\tt cubMC\_g}}\label{sec:cubmcgnumericalexample}
