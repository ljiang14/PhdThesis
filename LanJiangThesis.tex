...
\caption{Execution times and errors for Multivariate Normal Probability test function \eqref{MVNP} with the dimension $d$ IID uniform in $[2,8]$. The tolerance is is $\max\left(10^{-3},10^{-4} \abs{I}\right)$. All other settings are same as in Figure \ref{fig:productfunreltol}. \label{fig:MVNPTestFun1} }
\end{figure}
...
\end{align}
 IID Bernoulli random samples, where $\alpha_i=1-(1-\alpha/2)^{2^{-i}}$.
\item \label{step2}
\begin{enumerate}
\item If 
...
\end{align}
There are two random parts in $n_{\tot}$, the stopping time $\tau$ and the lower bound of $p$, $\hp_L$. In order to find its upper bound, we need the upper bound of $\tau$ and lower bound of $\hp_L$. Next, define the upper bound on the stopping time as
...
\begin{align}
n_{\up} = \sum_{i=1}^{\bar{\tau}} \left \lceil -\frac{4^i \log(\alpha_i)} {2\varepsilon_r^2 } \right\rceil+\left \lceil -\frac{\log (\alpha/4)}{2  (\varepsilon_r \varepsilon_{r})^2} \right \rceil.
\end{align}
...
\end{align*}
by the definition of $\bar{\tau}$ in \eqref{taubarBerRel}, we have:
\begin{align*}
\Pr (\tau \leq \bar{\tau}) &\geq \Pr (\hp_{n_i} \geq  p-\varepsilon_r2^{-i}, i \in \naturals )\\&
